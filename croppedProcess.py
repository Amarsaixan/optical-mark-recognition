...
def check(ff):
	...
	for i in range(mor,rows,mor):
		for j in range(bagana,cols,bagana):
			...
			if isFilled(crop_img,0,0):
				#print("filled", str(i)+","+str(j))
				print(str(i)+' -- '+str(j))
				cv2.rectangle(blank_image,(j, i),(bagana, mor),(random.randint(1,255),random.randint(1,255),random.randint(1,255)),8)
				for i1 in range(i, i+mor, 1):
					for j1 in range(j, j+bagana, 1):
						blank_image[i1, j1]=255
			else:
				correct+=1
